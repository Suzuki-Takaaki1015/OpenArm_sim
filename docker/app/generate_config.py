...
def body(b,parent):
    ...
    if js:
        ...
        effort=(15 if slide else 7) if finger else (40 if jname.endswith(('joint1','joint2')) else 27 if jname.endswith(('joint3','joint4')) else 7)
        velocity=(0.02 if slide else 0.5) if finger else 1.0
        joint=E.SubElement(robot,'joint',name=jname,type='prismatic' if slide else 'revolute')
        ...
        # v1 has independent slides; v2 finger 2 follows finger 1 mechanically.
        if V2 and jname.endswith('finger_joint2'):
            E.SubElement(joint,'mimic',joint=jname[:-1]+'1',multiplier='1',offset='0')
        else:
        ...
        # The topic hardware plugin requires names and command arrays to align.
        # Passive mimic joints stay in URDF, outside ros2_control interfaces.
        if V2 and jname.endswith('finger_joint2'):hardware.remove(ctl)
        joint_limits[jname]={'has_velocity_limits':True,'max_velocity':velocity,'has_acceleration_limits':True,'max_acceleration':(0.05 if slide else 1.0) if finger else 1.6}
...
for side in ('left','right'):
    fingers=[f'openarm_{side}_ee_inner_finger',f'openarm_{side}_ee_outer_finger'] if V2 else [f'openarm_{side}_left_finger',f'openarm_{side}_right_finger']
    E.SubElement(semantic,'disable_collisions',link1=fingers[0],link2=fingers[1],reason='Gripper contact')
    for finger in fingers:
        E.SubElement(semantic,'disable_collisions',link1=f'openarm_{side}_ee_base_link' if V2 else f'openarm_{side}_link7',link2=finger,reason='Gripper mount')
...
for side in ('left','right'):
    for suffix in ('arm','gripper'):
        ...
        if suffix=='arm':
            E.SubElement(g,'chain',base_link=f'openarm_{side}_base_link' if V2 else f'openarm_{side}_link0',tip_link=f'openarm_{side}_hand')
        ...
        if suffix=='gripper':
            ...
            for name in names:E.SubElement(state,'joint',name=name,value=str((.65 if side=='left' else -.65) if V2 else .03))
        if V2 and suffix=='gripper':names=names[:1]
        ...
        tolerance=(0.04 if V2 else 0.005) if suffix=='gripper' else 0.04
        controllers[ctl]={'ros__parameters':{'joints':names,'command_interfaces':['position'],'state_interfaces':['position','velocity'],'allow_partial_joints_goal':False,'constraints':{'goal_time':5.0,'stopped_velocity_tolerance':0.1,**{n:{'trajectory':0.2 if suffix=='arm' or V2 else 0.02,'goal':tolerance} for n in names}}}}
