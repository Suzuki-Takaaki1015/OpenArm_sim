...
NAME = 'openarm-auto'
WORKSPACE = '/workspaces/OpenArm_sim'
...
def main():
    ...
    if not info['State']['Running']:
        raise RuntimeError('OpenArm is stopped. Run python3 start.py from your checkout first.')
    ...
    if not any(m['Destination'] == WORKSPACE and m.get('RW') and m['Source'] == owner for m in info['Mounts']):
        raise RuntimeError('The development folder is not mounted. Restart once with python3 start.py in ' + owner)
    ...
    print('[OK] Container: ' + NAME + '; files saved to ' + owner, flush=True)
    if args.check:
